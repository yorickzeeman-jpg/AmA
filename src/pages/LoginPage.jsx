...
// Enterprise login — Discovery Navy / AEB Orange theme
// Shows email/password form. For demo: click a user card to sign in instantly.
...
export default function LoginPage({ onLogin }) {
  ...
  function handleSignIn() {
    ...
    setTimeout(() => {
      ...
      const u = INITIAL_USERS.find(x => x.email.toLowerCase() === email.toLowerCase())
      if (u) { onLogin(u) }
      else { setLoading(false); setError('No account found for this email address. Use the demo access below.') }
    }, 900)
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 20px', position:'relative', zIndex:5 }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:40, maxWidth:900, width:'100%', alignItems:'start' }}>

          {/* Left hero */}
          <div style={{ paddingTop:20 }}>
            <div style={{ fontSize:'clamp(44px,6vw,72px)', fontWeight:900, lineHeight:0.95, letterSpacing:'-2px', marginBottom:10 }}>
              <span style={{ color:'#fff' }}>AEB </span>
              <span style={{ color:T.orange }}>Portal</span>
            </div>
            <div style={{ fontSize:15, color:'rgba(255,255,255,0.5)', letterSpacing:'1px', textTransform:'uppercase', marginBottom:18, fontWeight:500 }}>Employer Services Platform</div>
          ...
          <div style={{
            ...
            background:'rgba(15,35,71,0.8)', backdropFilter:'blur(24px)',
            ...
            <div style={{ padding:'28px 28px 24px' }}>
              ...
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:10, fontWeight:700, color:'rgba(139,164,200,0.8)', textTransform:'uppercase', letterSpacing:'0.8px', marginBottom:6 }}>Email Address</div>
                <div style={{ position:'relative' }}>
                  ...
                  <input value={email} onChange={e=>{setEmail(e.target.value);setError('')}} onKeyDown={e=>e.key==='Enter'&&handleSignIn()} type="email" placeholder="Enter your email address"
              ...
              <div style={{ marginBottom:14 }}>
                ...
                <div style={{ position:'relative' }}>
                  ...
                  <input value={password} onChange={e=>{setPassword(e.target.value);setError('')}} onKeyDown={e=>e.key==='Enter'&&handleSignIn()} type={showPw?'text':'password'} placeholder="Enter your password"
                  ...
                  <button onClick={()=>setShowPw(p=>!p)} style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'rgba(139,164,200,0.5)', display:'flex' }}>
              ...
              {error && <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:8, padding:'8px 12px', fontSize:12, color:'#fca5a5', marginBottom:14 }}>{error}</div>}

              {/* Sign In button */}
              <button onClick={handleSignIn} disabled={loading} style={{
                width:'100%', padding:'14px',
                background:loading?'rgba(232,104,10,0.6)':'linear-gradient(135deg,#e8680a,#c95500)',
                border:'none', borderRadius:9, color:'#fff',
                fontSize:14, fontWeight:700, letterSpacing:'0.5px',
                cursor:loading?'not-allowed':'pointer',
                display:'flex', alignItems:'center', justifyContent:'center', gap:8,
                boxShadow:'0 4px 20px rgba(232,104,10,0.3)',
                transition:'all .2s', fontFamily:'inherit',
              }}>
                {loading
                  ? <><svg width="16" height="16" viewBox="0 0 24 24" fill="white" style={{animation:'spin 1s linear infinite'}}><path d="M12 4V2A10 10 0 0 0 2 12h2a8 8 0 0 1 8-8z"/></svg>Authenticating…</>
              ...
              <div style={{ marginTop:18, paddingTop:16, borderTop:'1px solid rgba(255,255,255,0.07)' }}>
                <button onClick={()=>setShowDemo(d=>!d)} style={{ background:'none', border:'none', color:'rgba(139,164,200,0.6)', fontSize:11, cursor:'pointer', display:'flex', alignItems:'center', gap:5, margin:'0 auto', fontFamily:'inherit' }}>
                  ...
                  Demo access {showDemo ? '▲' : '▼'}
                </button>
                {showDemo && (
                  <div style={{ marginTop:10, display:'flex', flexDirection:'column', gap:6 }}>
                    {INITIAL_USERS.map(u => (
                      <button key={u.id} onClick={()=>onLogin(u)} style={{ padding:'8px 12px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10, transition:'all .12s', fontFamily:'inherit' }}
                        onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,255,255,0.08)'}}
                        onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.04)'}}>
                        <div style={{ width:28, height:28, borderRadius:'50%', background:ROLES.find(r=>r.id===u.role)?.color||T.orange, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color:'#fff', flexShrink:0 }}>{u.avatar}</div>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontSize:12, fontWeight:600, color:'#fff' }}>{u.name}</div>
                          <div style={{ fontSize:10, color:'rgba(139,164,200,0.6)' }}>{u.role.replace(/_/g,' ')}</div>
                        </div>
                      </button>
                    ))}
                  </div>
      ...
      <div style={{ textAlign:'center', padding:'14px', borderTop:'1px solid rgba(255,255,255,0.05)', background:'rgba(7,18,42,0.8)', position:'relative', zIndex:5 }}>
        <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)' }}>AEB Portal is a secure platform powered by <span style={{ color:'rgba(255,255,255,0.5)' }}>Amadwala Employee Benefits</span> · © 2024</div>
      </div>
