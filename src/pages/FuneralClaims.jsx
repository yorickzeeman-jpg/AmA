...
['details','4', 'Deceased & Claimant'],
['summary','5', 'Claim Summary'],
['complete','6','Registered'],
].map(([id, num, label], i, arr) => {
const steps = ['select','upload','search','details','summary','complete']
...
{/* ── STEP 4: DECEASED & CLAIMANT DETAILS ── */}
{step==='details' && (
<div style={{ maxWidth:720, margin:'0 auto', width:'100%' }}>
{/* Member context */}
{foundMember && (
...
<div>
<div style={{ fontSize:10, fontWeight:700, color:T.gray, textTransform:'uppercase', marginBottom:2 }}>Verified Main Member</div>
<div style={{ fontSize:14, fontWeight:800, color:'#059669' }}>{foundMember.memberName} {foundMember.surname||''}</div>
<div style={{ fontSize:11, color:T.gray }}>
{foundMember.payrollNumber&&`Payroll: ${foundMember.payrollNumber}`}{foundMember.idNumber&&` · ID: ${foundMember.idNumber}`}
</div>
</div>
{claimType==='main_member' && (
<button onClick={()=>{
S('deceasedName', `${foundMember.memberName||''} ${foundMember.surname||''}`.trim())
S('deceasedId', foundMember.idNumber||'')
S('relationship', 'Main Member')
}}
style={{ padding:'8px 14px', background:'#059669', border:'none', borderRadius:8, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
Deceased is the main member — auto-fill
</button>
)}
</div>
)}
...
{/* Deceased details */}
<Card title="Deceased Details" color={T.blue}>
...
disabled={!claimData.deceasedName||!claimData.dateOfDeath}
style={{ flex:2, padding:'12px', background:T.orange, border:'none', borderRadius:10, color:'#fff', fontSize:14, fontWeight:700, cursor:!claimData.deceasedName||!claimData.dateOfDeath?'not-allowed':'pointer', fontFamily:'inherit', opacity:!claimData.deceasedName||!claimData.dateOfDeath?0.4:1 }}>
Next → Claim Summary
</button>
</div>
</div>
...
<div style={{ display:'flex', gap:10 }}>
<button onClick={()=>setStep('details')} style={{ flex:1, padding:'12px', background:'#fff', border:`1px solid ${T.border}`, borderRadius:10, color:T.text, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>← Back</button>
